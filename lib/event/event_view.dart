...
class _EventViewState extends State<EventView> {
  ...
  // danh sách sự kiện
  List<EventModal> items = [];

  // Tạo calenderController để điều khiển Sfaclendar
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SfCalendar(
        ...
        onLongPress: (detals) {
          if (detals.targetElement == CalendarElement.calendarCell) {
            final newEvent = EventModal(
                startTime: detals.date!,
                endTime: detals.date!.add(const Duration(hours: 1)),
                subject: 'Sự kiện mới');
        ...
        onTap: (details) {
          if (details.targetElement == CalendarElement.appointment) {
            final EventModal event = details.appointments!.first;
            Navigator.of(context).push(MaterialPageRoute(
              builder: (context) {
